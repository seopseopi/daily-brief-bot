...
[`morning.yml`](../.github/workflows/morning.yml)은 KST **07:30, 07:40, 07:50**에 실행 기회를 만듭니다. 같은 실행 그룹은 직렬로 처리하고, 첫 성공 날짜를 `data/delivery_state.json`에 기록합니다. 같은 날짜의 후속 **예약 실행**은 건너뜁니다.

GitHub Actions 예약은 정시 실행 SLA가 없어 혼잡 시 늦거나 누락될 수 있습니다. 07:30 정시가 필수인 환경은 외부 스케줄러 또는 서버 cron에서 실행하고 성공한 전송 상태를 다음 실행까지 보존해야 합니다.
...
| 07:30보다 늦게 도착 | Actions의 예약 시각과 실제 시작 시각. cron은 정시 보장 없음 |
