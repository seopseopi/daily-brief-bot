...
JSON의 `coverage`에는 실제 기사 분야 수, 일정 수, 설정 대비 보유종목 수, 논문·개념·용어 존재 여부 등을 포함합니다. 고정 시간표도 정상적인 개인 캘린더로 인정하므로 별도의 iCal 연동은 필요하지 않습니다. `optional_unconfigured`는 사용할 일정 소스가 하나도 없을 때만 `schedule`을 표시합니다. 정상 0건인 학사공지는 브리핑에서 숨기지 않고 새 공지 없음으로 표시합니다.
