...
from sources import _arxiv, _community, _discord, _llm, _market, _notices, _rss, _translate
...
def _load_discord_assignment_state():
    try:
...
def _save_discord_assignment_state(state):
...
def _fetch_discord_assignments():
    """#과제입력 채널의 새 메시지를 LLM으로 파싱해 과제 목록에 누적한다.

    data/discord_assignments.json에 마지막으로 읽은 메시지 ID와 지금까지
    파싱된 과제를 저장해둔다 — 매번 채널 전체를 다시 파싱하지 않기 위해서.
    ...
    state = _load_discord_assignment_state()
    ...
    try:
        ...
        _fail("과제 자동파싱(LLM)")
        # 파싱은 실패해도 last_message_id는 갱신 — 다음 실행 때 같은 메시지를
        # 계속 재시도하며 실패를 반복하는 것보다, 그냥 다음 새 메시지부터
        # 다시 시도하는 편이 낫다고 판단.
    ...
    _save_discord_assignment_state({"last_message_id": messages[-1]["id"], "assignments": assignments})
    return assignments


def get_assignments():
    """sources/assignments_data.py 수동 목록 + #과제입력 채널 자동 파싱 결과를 합쳐 D-day로 변환.

    수동 목록은 새 과제가 생기면 그 파일에 한 줄 추가하는 방식(그대로 유지).
    디스코드 채널에 타이핑하면 LLM이 과제명·마감일을 자동으로 뽑아 누적한다.
